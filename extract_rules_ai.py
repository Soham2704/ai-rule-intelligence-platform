...
You must extract rules into a list of JSON objects, where each object has the following keys: "id", "city", "rule_type", "conditions", "entitlements", "notes", "authority", "clause_no", "page".

1.  **id**: A unique ID you generate, like "MUM-FSI-002".
2.  **city**: The city the rule applies to (e.g., "Mumbai").
3.  **rule_type**: A camel-case category (e.g., "FSI", "Setback").
4.  **conditions**: A JSON object describing the "IF" part of the rule. For numerical conditions, use `{{ "min": X, "max": Y }}`.
5.  **entitlements**: A JSON object describing the "THEN" part of the rule.
6.  **notes**: A brief, human-readable summary of the rule.
7.  **authority**: The governing body mentioned (e.g., "MCGM"). If not mentioned, use "Unknown".
8.  **clause_no**: The specific clause number from the text (e.g., "12.3"). If not found, use "N/A".
9.  **page**: The page number of the source document, which is provided to you.
...
**Instructions:**
* Analyze the <TEXT_BLOCK> provided below. It is from page number **{page_number}**.
...
class RuleExtractionAgent:
    ...
    def extract_rules_from_text(self, text_chunk: str, city: str, page_number: int):
        response = self.chain.invoke({"text_chunk": text_chunk, "page_number": page_number})
        try:
            ...
            if start_index != -1 and end_index != 0:
                ...
                for rule in extracted_data:
                    ...
                    if 'page' not in rule: rule['page'] = str(page_number)
...
def process_page(page_data, city_name, agent):
    ...
    return agent.extract_rules_from_text(text_content, city_name, page_number)
...
def run_extraction_pipeline(input_path: str, city_name: str):
    ...
    unique_rules = {rule.get("id"): rule for rule in all_extracted_rules if rule.get("id")}
    ...
    if not final_rules_to_commit:
        print("No new rules to commit."); return
    ...
    try:
        ...
        for rule_data in tqdm(final_rules_to_commit, desc="Saving to DB"):
            ...
            required_keys = ["id", "city", "rule_type", "conditions", "entitlements", "notes", "authority", "clause_no", "page"]
            if not all(key in rule_data for key in required_keys): 
                print(f"  - WARNING: Skipping rule '{rule_id}' due to missing keys.")
                continue
        ...
        print(f"\n!!! An error occurred: {e}"); db.rollback()
    finally:
        db.close(); print("Database session closed.")
    print(f"\n--- Curation Complete for {city_name} ---")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract rules from an OCR'd JSON file and load them into the database.")
